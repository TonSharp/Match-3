...
public class TilesEditor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.C))
            Debug.Log(_integrity.IsCompleted());

        if (Input.GetKeyDown(KeyCode.F) && _integrity.IsCompleted())
            StartFill();
    ...
    {
        var leftCorner = _integrity.FindTopLeftCorner();
        var start = GetFreeCell(leftCorner);
